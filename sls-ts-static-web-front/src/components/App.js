...
const baseURL = 'https://tel9au775k.execute-api.ap-northeast-2.amazonaws.com/prod';
...
export default class extends React.Component {
  ...
  handleChangeItem(e, i) {
    ...
    nextItems[i].content = e.target.value;
              ...
              this.state.items.map((item, i) => (
                <div>
                  <StyledListItem key={i}>
                    <TextField name={`content${i}`} value={item.content} fullWidth onChange={(e) => this.handleChangeItem(e, i)}/>
                    <TextField readOnly name={`content${i}`} value={"MeCab 결과: " + JSON.stringify(item.mecabResult)} fullWidth onChange={(e) => this.handleChangeItem(e, i)}/>
                    <ListItemSecondaryAction>
                      <StyledRaisedActionButton label="수정" primary={true} onClick={() => this.updateItem(item.createdAt, item.content)}/>
